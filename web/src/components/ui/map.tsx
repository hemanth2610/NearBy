...
export const cartoDarkStyle: MapLibreGL.StyleSpecification = {
  version: 8,
  sources: {
    "carto-dark-tiles": {
      type: "raster",
...
export const cartoLightStyle: MapLibreGL.StyleSpecification = {
  version: 8,
  sources: {
    "carto-light-tiles": {
      type: "raster",
  ...
  return (
    <div className="bg-background/50 absolute inset-0 z-10 flex items-center justify-center backdrop-blur-xs">
      <div className="flex gap-1">
        <span className="bg-muted-foreground/60 size-1.5 animate-pulse rounded-sm" />
        <span className="bg-muted-foreground/60 size-1.5 animate-pulse rounded-sm [animation-delay:150ms]" />
        <span className="bg-muted-foreground/60 size-1.5 animate-pulse rounded-sm [animation-delay:300ms]" />
      </div>
    </div>
  );
}
...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  useEffect(() => {
    ...
    // Fallback timeout to unblock map UI if tile loading is delayed or offline
...
function DefaultMarkerIcon() {
  return (
    <div className="relative h-4 w-4 rounded-sm border-2 border-white bg-blue-500 shadow-lg" />
  );
}
...
function MarkerPopup({
  ...
  return createPortal(
    <div
      className={cn(
        "bg-popover text-popover-foreground relative max-w-62 rounded-sm border p-3 shadow-md",
...
function MarkerTooltip({
  ...
  return createPortal(
    <div
      className={cn(
        "bg-foreground text-background pointer-events-none rounded-sm px-2 py-1 text-xs text-balance shadow-md",
        "animate-in fade-in-0 zoom-in-95 duration-200 ease-out",
        className,
      )}
    >
...
function ControlGroup({ children }: { children: React.ReactNode }) {
  return (
    <div className="border-border bg-background [&>button:not(:last-child)]:border-border flex flex-col overflow-hidden rounded-sm border shadow-sm [&>button:not(:last-child)]:border-b">
      {children}
    </div>
  );
}
